...
class PlanController extends Controller
{
	...
	public function searchPlans($type, $service, $ldc, $promo = null){
		...
		if($promo === null){
			$ps = \App\Models\Plan::orderBy('length', 'desc')->where('ldc', $ldc)->where('type', $service)->whereNull('promo')->get();
		}
		else{
			$ps = \App\Models\Plan::orderBy('length', 'desc')->where('ldc', $ldc)->where('type', $service)->where('promo', $promo)->get();
		}
	...
	public function internalPlans($service, $ldc){
		...
		$ps = \App\Models\Plan::orderBy('length', 'desc')->where('ldc', $ldc)->where('type', $service)->get();
